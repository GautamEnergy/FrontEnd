...
const CardComponent = ({ imageSrc, header, source, date, url,author }) => {
  ...
  return (
    ...
    <Stack direction="row" spacing={4}>
      ...
      <Box _hover={{cursor:'pointer',textDecoration:'underline'}} flexGrow={1} paddingTop={{base:'9px',md:'32px',lg:'50px'}} onClick={openNewTab}>
        <Heading color={'grey'} display={{base:'none',md:'block',lg:'block'}} fontSize={'15px'}>{`by ${source} | ${Date(date).split(' ')[0]} ${Date(date).split(' ')[1]} ${Date(date).split(' ')[2]} | ${author || ''}`}</Heading>
